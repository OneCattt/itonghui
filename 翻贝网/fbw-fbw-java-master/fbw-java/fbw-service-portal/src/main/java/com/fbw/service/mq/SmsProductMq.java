...
public class SmsProductMq
{
    ...
    @Resource(name = "sendVoiceMsg")
    private Queue sendVoiceMsg;
     ...
     * @see [类、类#方法、类#成员]
     */
    public void voiceSmsSend(String reqData)
    {
        this.jmsMessagingTemplate.convertAndSend(this.sendVoiceMsg, reqData);
    }
    ...
    private void noticeSms(String smsmodel, Map<String, Object> reqMap) throws InnerException
    {
        ...
        if (SMS_64580023.equals(smsmodel) || SMS_64535035.equals(smsmodel))
        {
            ...
            smsParam = value.replace("*shopname*", shopname).replace("*cardholder*", cardholder).replace("*bankcard*",
                    bankcard);
        }
        if (SMS_48320121.equals(smsmodel))
        {
            ...
            smsParam = value.replace("*shopname*", shopname).replace("*money*", money).replace("*banknumber*",
                    banknumber);
        }
        if (SMS_39190380.equals(smsmodel) || SMS_39335358.equals(smsmodel))
        {
            ...
            smsParam = value.replace("*name*", String.valueOf("name"));
        }
        if (SMS_34870048.equals(smsmodel))
        {
            ...
            smsParam = value.replace("*money*", String.valueOf("money"));
        }
        if (SMS_1103010002.equals(smsmodel))
        {
            String methodName = String.valueOf(reqMap.get("methodName"));
            if ("null".equals(methodName))
            {
                throw new InnerException("2001", smsmodel + InnerCode.geterrorMsg("2001"));
            }
            smsParam = value.replace("*methodName*", methodName);
        }
    }
