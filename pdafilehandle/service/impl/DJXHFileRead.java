 ...
 * @File Name: DJCYFileRead：车辆送货：入库卸货txt读取
 * @Last version: 1.0
 * @Function:
 * @Last Update Date:
 * @Change Log:
 */
@Service("DJXHFileRead")
public class DJXHFileRead implements FileReadWay {

  private static final String SYS_NO = "b2b2c biz DJXHFileRead";
    ...
    try {
      ...
      while ((temp = reader.readLine()) != null) {
        ...
        if (pdaStorageApplicationMapper.selectWhetherHaveIntNumByBarCode(map)>0) {
          ...
          pdaFileHandleMo.setMass(Double.parseDouble(list[4]));
          pdaFileHandleMos.add(pdaFileHandleMo);
        }
      }
      if (pdaFileHandleMos.size() > 0) {
        int flag = pdaStorageApplicationMapper.insertDJXHBatches(pdaFileHandleMos);
