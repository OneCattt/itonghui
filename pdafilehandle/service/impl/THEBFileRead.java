 ...
 * @File Name: DJCYFileRead：车辆提货：空载过磅txt读取
 * @Last version: 1.0
 * @Function:
 * @Last Update Date:
 * @Change Log:
 */
@Service("THEBFileRead")
public class THEBFileRead implements FileReadWay {
  private static final String SYS_NO = "b2b2c biz THEBFileRead";
  ...
  public int saveTxt(File file) {
    ...
    try {
      ...
      if (pdaFileHandleMos.size() > 0) {
        int flag=pdaStorageApplicationMapper.insertTHEBBatches(pdaFileHandleMos);
