 ...
 * @File Name: DJCYFileRead：车辆送货：车辆入园txt读取
 * @Last version: 1.0
 * @Function:
 * @Last Update Date:
 * @Change Log:
 */
@Service("DJRYFileRead")
public class DJRYFileRead implements FileReadWay {

  private static final String SYS_NO = "b2b2c biz DJRYFileRead";
    ...
    try {
      ...
      while ((temp = reader.readLine()) != null) {
        ...
        if (pdaStorageApplicationMapper.selectWhetherHaveIntNumByBarCode(map)>0) {
          ...
          pdaFileHandleMo.setContainerNo(list[4]);
          pdaFileHandleMos.add(pdaFileHandleMo);
        }
      }
      if (pdaFileHandleMos.size() > 0) {
        int flag = pdaStorageApplicationMapper.insertDJRYBatches(pdaFileHandleMos);
