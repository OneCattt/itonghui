 ...
 * @File Name: DJCYFileRead：车辆提货：入库提货txt读取
 * @Last version: 1.0
 * @Function:
 * @Last Update Date:
 * @Change Log:
 */
@Service("THPGFileRead")
public class THPGFileRead implements FileReadWay {
  private static final String SYS_NO = "b2b2c biz THPGFileRead";
  ...
  public int saveTxt(File file) {
    ...
    try {
      ...
      if (pdaFileHandleMos.size() > 0) {
        int flag=pdaStorageApplicationMapper.insertTHPGBatches(pdaFileHandleMos);
