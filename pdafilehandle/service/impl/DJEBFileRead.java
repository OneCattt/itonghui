 ...
 * @File Name: DJCYFileRead：车辆送货：空载过磅txt读取
 * @Last version: 1.0
 * @Function:
 * @Last Update Date:
 * @Change Log:
 */
@Service("DJEBFileRead")
public class DJEBFileRead implements FileReadWay {

  private static final String SYS_NO = "b2b2c biz DJEBFileRead";
  ...
  public int saveTxt(File file) {
    ...
    StorageApplicationMapper storageApplicationMapper = SpringApplicationContextUtil
        .getBean(StorageApplicationMapper.class);
    ...
    try {
      ...
      while ((temp = reader.readLine()) != null) {
        ...
        if (pdaStorageApplicationMapper.selectWhetherHaveIntNumByBarCode(map)>0) {
	        ...
	        pdaFileHandleMo.setEmWeight(Double.parseDouble(list[4])/1000);// kg转吨
	        pdaFileHandleMo.setNetWeight(Double.parseDouble(list[4])/1000);// kg转吨
	        pdaFileHandleMo.setEmWeight(Double.parseDouble(list[5])/1000);// kg转吨
	        pdaFileHandleMos.add(pdaFileHandleMo);
        }
      }
      if (pdaFileHandleMos.size() > 0) {
        int count = DJCYFileRead
            .basicDataDeal(pdaStorageApplicationMapper, storageApplicationMapper, pdaFileHandleMos,
                SYS_NO);
        if(count < 1){
        	throw new BusinessException(SYS_NO + " 0002", "平台入库数量操作失败");
        }
        int flag = pdaStorageApplicationMapper.insertDJEBBatches(pdaFileHandleMos);
